...
@protocol CLClippingPanelDelegate;
@interface CLClippingPanel : UIView
@property (nonatomic, assign) CGRect clippingRect;
@property (nonatomic, strong) CLRatio *clippingRatio;
@property (nonatomic, weak) id<CLClippingPanelDelegate> delegate;
- (id)initWithSuperview:(UIView*)superview frame:(CGRect)frame andDelegate:(id<CLClippingPanelDelegate>)delegate;
...
@protocol CLClippingPanelDelegate <NSObject>
@required
- (void) clClippingPanel:(CLClippingPanel*)panel didSelectEnd:(CGRect)rect;
@end
...
@interface CLClippingTool() <CLClippingToolDelegate>
...
- (void) setupMemberRect:(CGRect)rect {
    [_gridView setClippingRect:rect];
}
...
- (void)setup
{
    ...
    _gridView = [[CLClippingPanel alloc] initWithSuperview:_imageView.superview frame:_imageView.frame andDelegate:self];
...
{
    CGFloat zoomScale = _imageView.width / _imageView.image.size.width;
    ...
    UIImage *result = [_imageView.image crop:rct];
...
- (void)executeWithCustomCompletionBlock:(void (^)(UIImage *image, CGRect rectOriginal, CGRect cropRectSetting))completionBlock {
    ...
    UIImage *result = [_imageView.image crop:rct];
    completionBlock(result, rct, cropRect);
}
...
- (void)setCropMenu
{
    ...
    UIImage *iconImage = [_imageView.image resize:CGSizeMake(W * imgSize.width/maxW, W * imgSize.height/maxW)];
...
#pragma mark - CLClippingPanelDelegate
- (void) clClippingPanel:(CLClippingPanel*)panel didSelectEnd:(CGRect)rect {
    [self.delegate clClippingTool:self didMoveSelectGridEnd:rect];
}
@end
...
- (void)panCircleView:(UIPanGestureRecognizer*)sender
{
    ...
    if (sender.state == UIGestureRecognizerStateEnded) {
        NSLog(@"Rect: %@", NSStringFromCGRect(rct));
        [self.delegate clClippingPanel:self didSelectEnd:rct];
    }
}
